  ...
  // Step 1: Append tooltip div
    ...
    circlesGroup.on("mouseover", function(d) {
      toolTip.style("display", "block")
          .html(
            // `<strong>${d.DIABETES_CASES} Cases<strong><hr>${d[datapoint1]}${y1_axis_name}`)
          `<strong>${d.Geographic_Area_Name}<hr>${d.DIABETES_CASES} Cases<strong><hr>${d[datapoint1]}${y1_axis_name}`)
...
function createToolTipforSecondChart(chartGroup, censusData, xLinearScale, yLinearScale2, initChartGroup, datapoint2, y2_axis_name){
  ...
  // append circles
    ...
    circlesGroup1.on("mouseover", function(d) {
      toolTip.style("display", "block")
          .html(
            `<strong>${d.Geographic_Area_Name}<hr>${d.DIABETES_CASES} Cases<strong><hr>${d[datapoint2]}
         ${y2_axis_name}`)
          .style("left", d3.event.pageX + "px")
          .style("top", d3.event.pageY + "px");
    })
