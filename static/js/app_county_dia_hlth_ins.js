...
censusData.forEach(function(data) {
  ...
  data.compFactor = +data.HEALTH_Insurance; // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  console.log(data.DIABETES_CASES)
  console.log(data.HEALTH_Insurance) // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
});
...
var toolTip = d3.tip()
  ...
  .offset([45, -30])
  .html(function(d) {
    return (d.Geographic_Area_Name + " DC:" +d.DIABETES_CASES+ " HS:" +d.HEALTH_Insurance) // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  });
