// Create plot orientation setup for Diabetes vs. Education (HS Dropout)
var svgWidth = 960;
var svgHeight = 500;
...
censusData.forEach(function(data) {
  ...
  data.compFactor = +data.EDUCATION_hsdropout; // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  console.log(data.DIABETES_CASES)
  console.log(data.EDUCATION_hsdropout) // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
});
...
var toolTip = d3.tip()
  ...
  .offset([45, -30])
  .html(function(d) {
    return (d.Geographic_Area_Name + " DC:" +d.DIABETES_CASES+ " HS:" +d.EDUCATION_hsdropout) // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  });
