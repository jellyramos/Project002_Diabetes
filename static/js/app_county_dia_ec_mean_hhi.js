...
censusData.forEach(function(data) {
  ...
  data.compFactor = +data.ECONOMIC_meanincome; // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  console.log(data.DIABETES_CASES)
  console.log(data.ECONOMIC_meanincome) // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
});
...
var toolTip = d3.tip()
  ...
  .offset([45, -30])
  .html(function(d) {
    return (d.Geographic_Area_Name + " DC:" +d.DIABETES_CASES+ " HS:" +d.ECONOMIC_meanincome) // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  });
